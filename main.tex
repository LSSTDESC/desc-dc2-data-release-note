...
\label{sec:truth-variable}

Characteristics of variable sources (stars and SNe) can be found in several tables. These tables are of two types:
\begin{enumerate}
    ...
    \begin{enumerate}
        ...
        \item they do not include columns which have to do with matching nor any concerning spatial partitioning
        \item each has a few extra columns specific to source type
...
There is a summary table for galaxies as well, similar to Truth-match but with some extra columns.

For details about file format and organization see \autoref{sec:representation-unmerged}. The complete schema for these tables is described \autoref{app:variable-truth-schema} 

\subsection{Intermediate Data Products from LSST Science Pipelines}

The calibrated exposures, background subtracted images, and source catalogs output from the single-epoch processing as well as the coadded images (and the corresponding calibrated exposures, background models, and detected sources; see \autoref{app:coadded-image-files}) for a small sky region (tracts 3828 and 3829; see \autoref{sec:representation} for definition) are available. These images are part of the direct outputs from the image processing using the LSST Science Pipelines (see \autoref{sec:processing}). The single-epoch and coaddition processes are described in Sec.~3.1 and Sec.~3.3 of \cite{10.1093/pasj/psx080}, respectively, as well as in Sec.~3.1 and Sec.~4.2 of \cite{2018arXiv181203248B}. 
The file format and the organization of files for the single-epoch and coadded images are also described in \autoref{sec:representation-intermediate} and \autoref{app:coadded-image-files}. 
...
Each of the four point source tables (star summary, star variability, SN summary, SN variability) is available in two physical formats: SQLite database file and parquet file.  The table schema is identical between the two formats. The galaxy summary is available only in Parquet, partitioned into healpix pixels.  

The point source Parquet files are smaller than the SQlite versions, particularly in the case of the stellar variability truth table.  Also the Parquet files, and only the Parquet files, may be accessed via the Python package \code{GCRCatalogs}. However applications needing to do large numbers of queries of star light curves will benefit from the efficiency provided by SQLite since such queries run about $\sim$100 times faster on the SQLite files. 
